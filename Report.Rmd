...
The following plot shows the number of reviews for each star rating. 

```{r open_close, include = TRUE}
restaurants %>% 
...
There were 3476 restaurants included in our analysis. The Strip is the neighborhood with the most restaurants at 613, and Anthem has the least with only 10 restaurants. However, we noticed during our exploratory analysis that while The Strip had the highest number of restaurants, it received the 3rd worst restaurant ratings on average. We found that the restaurant category that received the best ratings regardless of neighborhood was Sushi Bars, with an average rating of 3.8 stars. The restaurant category with the lowest average star rating was Fast Food, with an average rating of only 2.6 stars. From these findings, we can infer that we would have a better overall food experience in any neighborhood in Las Vegas if we chose to eat at a Sushi Bar, and the worst experience at a Fast Food restaurant.
