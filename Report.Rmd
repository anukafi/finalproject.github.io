...
```{r stephs_geo_plot, include = FALSE}
...
```{r sarah_stacked_bar, include = FALSE}
popular <- categories %>% 
...
```{r sarah_geo_plot, include = FALSE}
restaurants %>% 
  inner_join(popular) %>% 
  plot_ly(x = ~longitude, y = ~latitude, type = "scatter", mode = "markers",
          alpha = 0.9, 
...
### DON'T TOUCH!! SARAH'S STATS STUFF
...
This code includes a count of the restaurants broken down by neighborhood, and the breakdown of stars for each neighborhood. There is also a table showing the average number of reviews for restaurants in each neighborhood. 
